...
**Purpose**: Compile declarative JSON table specifications into
dplyr/tidyr R scripts that, when evaluated against `data`, produce
pre-formatted tibbles for ksTFL.
...
Calc/format functions are **bare names** in generated code (resolved
from the eval env). There is no `calc_fns` / `format_fns` registry.
...
- `parameter_stat`: single-pass `summarize` + reshape (temp `.cN`
  columns)

### Phase 3: Metadata extraction — DONE (caller-driven)

- `kst_extract_metadata` / `kst_apply_metadata` before eval
- `groups.format` documented as `format_map` hint (not auto-applied by
  compile)
...
- README, ARCHITECTURE, DSL_EXAMPLE, NEWS, REQUIREMENTS aligned with
  code
...
Two layouts compile and eval on synthetic data

SR-1 injection rejected

Validate-before-compile

Docs match bare-call + pre-eval metadata model
...
- Auto-apply `groups.format` inside a helper (still caller-driven
  compile)
...
- Denominators / distinct-subject counting remain user calc
  responsibility
