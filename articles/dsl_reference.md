...
Generated: `count_pct(AGE, denom = dplyr::n())` (no prep table).

#### `type: "n_distinct"` — distinct column in the cell
...
Generated: `count_pct(AGE, denom = dplyr::n_distinct(USUBJID))`.
Requires `"variable"` (SR-1 identifier).

#### `type: "data_n"` — subset of `groups.by` from analysis data

Use when the table is stratified more finely than the percent
denominator (e.g. columns `TRT01P × SEX`, denom over `TRT01P` only):
...
Omit `"distinct"` to use row count
([`dplyr::n()`](https://dplyr.tidyverse.org/reference/context.html)) in
the pre-aggregate. `"by"` is required and must be a subset of
`groups.by`.

#### `type: "external"` — population / big-N from the eval environment

**Keyed tibble** (usual ADSL N by arm):
...
Bind `adsl_n` (columns `TRT01P`, `N`) into the eval environment. With
`"by"`, `"value"` is required. The compiler emits a rename + `left_join`
and `denom = dplyr::first(.kst_dK)`.

**Scalar** (omit `"by"` / `"value"`):

``` json
"denominator": { "type": "external", "name": "N_total" }
```

Generated: `denom = N_total` (symbol resolved from the eval
environment).
...
- Hierarchical layouts use the same field on the single active
  statistic.

Worked compile+eval examples for every kind are in
[`vignette("getting_started")`](https://crow16384.github.io/ksTable/articles/getting_started.md).
