...
Use when the table is stratified more finely than the percent denominator
(e.g. columns `TRT01P × SEX`, denom over `TRT01P` only):
...
Omit `"distinct"` to use row count (`dplyr::n()`) in the pre-aggregate.
`"by"` is required and must be a subset of `groups.by`.
...
Bind `adsl_n` (columns `TRT01P`, `N`) into the eval environment. With `"by"`,
`"value"` is required. The compiler emits a rename + `left_join` and
`denom = dplyr::first(.kst_dK)`.
...
Generated: `denom = N_total` (symbol resolved from the eval environment).
...
- Hierarchical layouts use the same field on the single active statistic.
